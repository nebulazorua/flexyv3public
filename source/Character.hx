...
{
	public var bandaidSolution:Float = 0; // kms
	...
	public static function getJSON(charName:String,player:Bool=false):CharJson{
		...
		if(OptionUtils.options.potato<=1){
			var nooPath = 'assets/characters/mid/data/' + daCharPath.replace(pathBase,'');
			if(FileSystem.exists(nooPath)){
				daCharPath = nooPath;
			}
		}
		if(OptionUtils.options.potato<=0){
			var nooPath = 'assets/characters/low/data/' + daCharPath.replace(pathBase,'');
			if(FileSystem.exists(nooPath)){
				daCharPath = nooPath;
			}
		}
		...
		#else
		if (player && Assets.exists(playerPath, TEXT))
			daCharPath = playerPath;
		if (OptionUtils.options.potato <= 1)
		{
			var nooPath = 'assets/characters/mid/data/' + daCharPath.replace(pathBase, '');
			if (Assets.exists(nooPath, TEXT))
			{
				daCharPath = nooPath;
			}
		}
		...
		Reflect.setField(shit,'format',"andromeda");
	...
	public function setChar(newChar:String){
		switch (newChar)
		{
			...
			default:
			{
				...
				#else
					if (isPlayer && Assets.exists(playerPath, TEXT))
						daCharPath = playerPath;
				#end
				charPath=daCharPath;
	...
	public function setCharData(){
		if(charData!=null){
			...
			forceFocus = curCharacter=='devouredintro';
	...
	public function new(x:Float, y:Float, ?character:String = "bf", ?isPlayer:Bool = false, ?hasTexture:Bool=true)
	{
		...
		if(hasTexture){
			if(curCharacter.contains("flexy") || curCharacter.contains("dmp"))hasMic=true;
			...
			var json:CharJson = getJSON(character, isPlayer);
	...
	public function loadAnimations(){
		...
		try {
			...
			if(Cache.animData[curCharacter]!=null){
	...
	override function update(elapsed:Float)
	{
		if (animation.curAnim!=null){
			if (animation.curAnim.finished){
				if (animation.getByName(animation.curAnim.name + "-end")!=null && !debugMode)
		...
		if (bandaidSolution > 0)
			bandaidSolution -= elapsed;
		...
		if (!isPlayer)
		{
			if(animation.curAnim!=null){
				...
				if(!debugMode){
					if (holdTimer >= Conductor.stepCrochet * dadVar && bandaidSolution <= 0)
					{
						dance();
	...
	{
		if (!disabledDance && animation.curAnim != null && noIdleTimer <= 0 && bandaidSolution<=0)
		{
	...
	public function playAnim(AnimName:String, Force:Bool = false, Reversed:Bool = false, Frame:Int = 0):Void
	{
		...
		if(animation.getByName(AnimName)!=null){
			...
			if (isSinging){
				bandaidSolution = (Conductor.stepCrochet * 0.1) / 1000;
				holdTimer = 0;
			}
