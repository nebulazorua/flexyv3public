...
class OptionsState extends MusicBeatState
{
	...
	private function createDefault(){
		defCat = new OptionCategory("Default",[
			new OptionCategory("Gameplay",[
				...
				new StepOption("noteOffset", "Offset", 1, -10000, 10000, "ms", "", "Used to adjust for audio latency.\nOffsets the game's \"logical\" song position, aka the song position used to calculate stuff like the \"visual\" song position, beats and a hit note's judgement", true),
				new StepOption("visualOffset", "Visual Offset", 1, -10000, 10000, "ms", "", "Used to adjust for monitor/visual latency.\nChanges how early or late notes visually are by offsetting the game's \"visual\" song position", true),
				...
				new ToggleOption("opponentMode","Opponent Mode","Play the opponent's side!"),
				#if !NO_FREEPLAY_MODS
				...
				new CallbackOption("Calibrate Offset", function(option:CallbackOption){
					var song = new EngineData.SongData('Offset', 'gf', 0);
					PlayState.setFreeplaySong(song, 1);
					PlayState.seenCutscene = false;
					PlayState.calibrating = true;
			...
			new OptionCategory("Performance",[
				new ScrollOption("potato","Quality","Turns off certain features depending on the level.",0,2,["Low", "Mid", "High"]),
				#if desktop
	...
	override function create()
	{
		...
		optionDesc = new FlxText(5, FlxG.height-48,1175,"",20);
		...
		optionDescBG = new FlxSprite().makeGraphic(1, 1, FlxColor.BLACK);
		add(optionDescBG);
		#if html5
	...
	override function update(elapsed:Float)
	{
		...
		optionDescBG.setGraphicSize(Math.ceil(optionDesc.textField.textWidth) + 2, Math.ceil(optionDesc.height));
		optionDescBG.updateHitbox();
		optionDescBG.visible = optionDesc.visible;
		optionDescBG.y = optionDesc.y;
		optionDescBG.screenCenter(X);
		optionDescBG.x += 2;
