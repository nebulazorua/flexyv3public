 ...
 **/
 ...
 override function create()
 {
   ...
   for(change in velChanges){
     var time = change.startTime;
     var section = Math.floor(Conductor.getStep(time)/16);
     var gotAdded:Bool=false;
     var changeEvent: Section.Event = {
     ...
     for(event in _song.notes[section].events){
       if(Math.abs(event.time - time) < 10){
         event.events.push(changeEvent);
         gotAdded=true;
         break;
       }
     }
     if(!gotAdded){
       _song.notes[section].events.push({
         time: time,
         events:[changeEvent]
       });
     }
 ...
 function loadSong(daSong:String):Void
 {
   ...
   FlxG.sound.playMusic(CoolUtil.getSound('${Paths.inst(daSong,(PlayState.storyDifficulty==3?"Erect":""))}'), 0.6);

   // WONT WORK FOR TUTORIAL OR TEST SONG!!! REDO LATER
   vocals = new FlxSound().loadEmbedded(CoolUtil.getSound('${Paths.voices(daSong,(PlayState.storyDifficulty==3?"Erect":""))}'));
