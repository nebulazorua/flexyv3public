...
class ExtrasState extends MusicBeatState
{
		...
		*/
		...
		for(week in EngineData.weekData){
            ...
            for(song in week.songs)
				songNames.push(song.chartName.toLowerCase());
			...
			if (FileSystem.exists(songFolder))
			{
				...
				if(!songNames.contains(song.toLowerCase())){
					...
					if(hasMetadata){
						...
						trace(song, Paths.anyChart(song, song));
						if (FileSystem.exists(Paths.anyChart(song,song))){
							var song = Song.loadFromJsonManual(Paths.anyChart(song, song));
					...
					if(FileSystem.exists(Paths.anyChart(song,song)) && !hasCharts){
						hasCharts=true;
					}

					if(add && hasCharts)
						addSong(display==null?song.replace("-"," "):display,0,icon,song,'fiesta');
		...
		bg = new FlxSprite().loadGraphic(Paths.image(FlxG.random.bool(1) ?'alternateending':'oldmenubg/menuBG'));
		add(bg);
	...
	public function addSongData(songData:EngineData.SongData)
	{
		...
		#else
		var suffixes = ["", "-easy", "-hard", "-erect"];
		var conversion = [1, 0, 2, 3];
		var daDiffs:Array<Int> = [];
		for (idx in 0...suffixes)
		{
			var suff = suffixes[idx];
			var name = 'asset/songs/${songData.chartName.toLowerCase()}$suff';
			if (OpenFlAssets.exists(name))
			{
	...
	public function addSong(songName:String, weekNum:Int, songCharacter:String, ?chartName:String, ?preloadPath:String)
	{
		addSongData(new SongData(songName, songCharacter, weekNum, chartName, preloadPath));
	}
...
class ExtrasState extends MusicBeatState {
	...
	{
		super.create();
		var sowwy = new FlxText(0, 0, 0, "press any key to go back", 48);
		sowwy.setFormat(Paths.font("vcr.ttf"), 48, FlxColor.WHITE, LEFT, OUTLINE, FlxColor.PURPLE);
		sowwy.screenCenter(XY);
		sowwy.alpha = 1;
	}
