...
class ModManager {
  public var inUse:Array<String> = [];
  var usedMods:Array<Modifier>=[];
  ...
  public function registerModifiers(){
    ...
    setInUse("perspective"); // should always be used
    setInUse("reverse"); // should always be used
		setInUse("opponentSwap"); // should always be used (cus opponent mode)
  }
  ...
  public function getPath(diff:Float, vDiff:Float, column:Int, player:Int):Vector3{
    ...
    for(mod in usedMods){
      if(mod.movesShit())pos = mod.getPath(vDiff, pos, column, player, diff);
    }
  ...
  public function getNoteScale(note:Note):TypePoint{
    var def = note.scaleDefault;
    var scale = {x: def.x, y:def.y};
    for(mod in usedMods){
	...
	public function updateNote(note:Note, player:Int, scale:TypePoint, pos:Vector3){
    for(mod in usedMods){
  ...
  public function getReceptorPos(rec:Receptor, player:Int=0):Vector3{
	...
	public function getReceptorScale(rec:Receptor, player:Int = 0):TypePoint{
	...
	public function updateReceptor(rec:Receptor, player:Int, scale:TypePoint, pos:Vector3){
    for(mod in usedMods){
      mod.updateReceptor(rec, player, pos, scale);
    }
  }

  function setInUse(name:String){
    if(!inUse.contains(name)){
      inUse.push(name);
      var mod = definedMods.get(name);
      if(mod.parent!=null){
        for(dName in definedMods.keys()){
          if(definedMods.get(dName)==mod.parent){
            setInUse(dName);
            return;
          }
        }
